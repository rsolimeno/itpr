...
In this part of the series we're going to look at the White Pages.\index{White Pages} No, not your local telephone directory, but the packet radio directory known as the ``White Pages''. You help supply the information for ``WP'', and you can also use it to find the home BBS, QTH and zip code of your friends on packet.
...
Just like all other packet messages, messages addressed to WP are forwarded from BBS to BBS toward their destination. If a BBS operating with the W0RLI WP Server\index{WP Server} handles a query message, it will respond with any pertinent information that it has available. As a result, you might receive more than one response to your WP query.
...
If you move or change your home BBS, you should then make sure that you update the information for your call in the White Pages database. Use the \verb!NH!,\index{NH command} \verb!NQ!\index{NQ command} and \verb!NZ!\index{NZ command} commands to update the information. Making sure that the information in the White Pages is correct will help to get your messages delivered to the correct BBS.\index{BBS}